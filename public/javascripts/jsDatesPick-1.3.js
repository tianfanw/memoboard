/*
	Dingan Wang - modified version based on 
...
g_l = [];
g_l["MONTHS"] = [ "Janaury", "February", "March", "April", "May",
...
String.prototype.strpad = function() {
    return (!isNaN(this) && this.toString().length == 1) ? "0" + this
	    : this;
};
...
JsDatePick.prototype.addOnSelectedDelegate = function(
	aDelegatedFunction) {
...
JsDatePick.prototype.setOnSelectedDelegate = function(
	aDelegatedFunction) {
    if (typeof (aDelegatedFunction) == "function") {
...
JsDatePick.prototype.setRepopulationDelegate = function(
	aDelegatedFunction) {
    if (typeof (aDelegatedFunction) == "function") {
...
JsDatePick.prototype.resizeCalendar = function() {
    ...
    this.leftWallStrechedElement.style.height = newStrechedHeight
	    + "px";
    this.rightWallStrechedElement.style.height = newStrechedHeight
	    + "px";
    return true;
};
...
JsDatePick.prototype.populateFieldWithSelectedDate = function() {
    $("#" + this.oConfiguration.target).val(
...
JsDatePick.prototype.makeCalendar = function() {
    ...
    // add all child to the main frame - 7 parts.
	...
	if (this.oConfiguration.target != false) {
		...
		inputElement = document
			.getElementById(this.oConfiguration.target);

		aSpan = document.createElement("span");
		inputElement.parentNode.replaceChild(aSpan,
			inputElement);
		aSpan.appendChild(inputElement);

		inputElement.setAttribute("globalNumber",
			this.globalNumber);
		inputElement.onclick = function() {
		    JsDatePick.getCalInstanceById(
			    this.getAttribute("globalNumber"))
			    .showCalendar();
		};
		...
		JsDatePickBox.style.zIndex = this.initialZIndex
			.toString();
	...
	if (this.oConfiguration.target != null) {
	    ...
	    $("#" + this.oConfiguration.target).css("position",
		    "relative");
	    JsDatePickBox.style.position = "absolute";
...
JsDatePick.prototype.determineFieldDate = function() {
	...
	if (jQuery.trim(aField.val()).length == 0) {
		...
		divider = this
			.senseDivider(this.oConfiguration.dateFormat);
...
JsDatePick.prototype.senseDivider = function(aStr) {
    return aStr.replace("%d", "").replace("%j", "").replace("%m", "")
	    .replace("%M", "").replace("%n", "").replace("%F", "")
	    .replace("%Y", "").replace("%y", "").substr(0, 1);
};
...
JsDatePick.prototype.showCalendar = function() {
	...
	$(this.JsDatePickBox)
		.attr("globalCalNumber", this.globalNumber);
	$(this.JsDatePickBox)
		.mouseout(
			function() {
...
JsDatePick.prototype.getDOMCalendarStripped = function() {
    var d = document, boxMain, boxMainInner, clearfix, boxMainCellsContainer, tooltip, weekDaysRow, clearfix2, controlsBar;
    ...
    controlsBar = this.getDOMControlBar();
    this.makeDOMWeekDays(weekDaysRow);

    boxMainInner.appendChild(controlsBar);
...
JsDatePick.prototype.populateMainBox = function(aMainBox) {
    ...
    // get the first data of the month Date(year, month, day, hours, minutes,
    // seconds, milliseconds)
    // oDay is the beginning day of this month
    ...
    skipDays = oDay.getDay() - this.oConfiguration.weekStartDay;
    // skipDays = parseInt(oDay.getDay())
    // - this.oConfiguration.weekStartDay;
    if (skipDays < 0) {
	skipDays = skipDays + 7;
    }
...
JsDatePick.prototype.getUnSelectedDay = function() {
	return this.unSelectedDayObject;
}
...
JsDatePick.prototype.getSelectedDayFormatted = function() {
	...
	dateStr = dateStr.replace("%d", this.selectedDayObject.day
		.toString().strpad());
	dateStr = dateStr.replace("%j", this.selectedDayObject.day);

	dateStr = dateStr.replace("%m", this.selectedDayObject.month
		.toString().strpad());
	dateStr = dateStr.replace("%M",
		g_l["MONTHS"][this.selectedDayObject.month - 1].substr(
			0, 3).toUpperCase());
...
JsDatePick.prototype.setDaySelection = function(anElement) {
	...
	if(parseInt(anElement.getAttribute("isSelected")) == -1){
		$(anElement).removeAttr("isSelected"); 
		return;
	}
	...
	anElement.style.background = "url("
		+ this.oConfiguration.imgPath + currentColorScheme
		// + "_dayDown.gif) left top no-repeat";
		+ "_dayDown.gif) left top repeat";
    } else {
...
JsDatePick.prototype.getDOMControlBar = function() {
	...
	// global number is used to locate element
	gRef = JsDatePick.getCalInstanceById(this
		.getAttribute("globalNumber"));
	gRef.setTooltipText(g_l["MONTH_FWD"]);
	gRef.setC(this, "monthForwardButtonOver");
    };

    /* Month forward button event handlers */
